...
class contacts
{
	...
	public function view()
	{
		...
		if (empty($controller->params[1])) {
			lib::sendto();
		} else {
			$params = new facadecontactinformation((int) $controller->params[1]);
	...
	public function edit()
	{
		...
		if (empty($controller->params[1])) {
			...
			$contact = new contact((int) $controller->params[1]);
	...
	public function deleteMethods(contact $contact)
	{
		...
		$sql = "delete g.*, m.* from contactgroup g left join contactmethod m on 
		g.id=m.contactgroupid where contactid={$contact->id}";
